...
网页支持录像上传、单视频与多局剪辑、全部公开参数、模板编辑、任务队列与取消、成片预览下载、慢回放复核、配音时间点编辑、模型与素材安装，以及 API 与算法配置。首页直接进入成片与素材列表，无需浏览后台文件目录；任务输出中可查看报告、恢复配置并重新渲染。无需 Node.js 或前端构建，详见[网页工作台](docs/网页工作台.md)。
