...
def api_decision(candidates, directory, top_k, focus, endpoint, model, key, timeout, reviews=None, audit=None):
    ...
    # Keep generation bounded as well as input context. An incomplete response
    # still fails closed into the rule path; never accept a truncated edit list.
    payload={'model':model,'max_tokens':8192,'messages':[{'role':'system','content':prompt},{'role':'user','content':content}],
