...
[volleymole.svg](branding/volleymole.svg) 是用户提供的原始标志的逐字节包内副本，并非新生成的插画。透明 PNG（`branding/volleymole.png`） 通过 `rasterize_logo.py` 使用 librsvg 按原始画布渲染，保留图形、颜色及宽高比，展示于每个转场顶部。`branding/volleymole.json` 记录 SVG、转换代码和 PNG 的 SHA-256；素材安装后直接复用已校验的 PNG，不需要修改 site-packages。开发时若替换包内 SVG，需同步重新生成派生资源并发布新的素材版本。
