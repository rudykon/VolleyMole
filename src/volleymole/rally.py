...
def build_manifest(source, analytics_path, ball_path, pts_path, player_path, directory, config, provenance):
    ...
    algorithm = config.get('rally_algorithm','legacy_v1')
    if algorithm not in ('legacy_v1','fused_v2'):
        raise ValueError('未知回合算法')
    origins = ['vball' if v else 'missing' for v in visible]
    associations = []
    if algorithm=='fused_v2':
        xy,visible,origins,associations = associate_auxiliary(records,xy,visible,t,w,
            config['auxiliary_ball_confidence'],config['auxiliary_max_gap_sec'])
        visible &= (xy[:,0]>=0)&(xy[:,0]<w)&(xy[:,1]>=0)&(xy[:,1]<h)
    ...
    raw_airborne = airborne & raw_visible & np.roll(raw_visible,1)
    low = np.zeros(len(t),dtype=bool)
    carried = np.zeros(len(t),dtype=bool)
    if algorithm=='fused_v2':
        low,carried = low_motion(records,xy,continuous,speed,np.asarray(heads),t,w,h,config['low_ball_min_speed'])
    motion = airborne | low
    ...
    for a, b, start in zip(lo, hi, edges):
        bins.append({'start_sec': float(start), 'end_sec': min(float(start+step), source['duration_sec']),
                     ...
                     'raw_flight_ratio':float(raw_airborne[a:b].mean()) if b>a else 0,
                     'supported_motion_ratio':float(motion[a:b].mean()) if b>a else 0,
                     'play_ratio': float(play[a:b].mean()) if b>a else 0})
    active = np.array([b['supported_motion_ratio'] >= config['flight_ratio'] for b in bins])
    ...
    boundary_guards = []
    if algorithm=='fused_v2':
        raw_active = np.array([b['raw_flight_ratio']>=config['flight_ratio'] for b in bins])
        raw_cores = grouped(np.flatnonzero(raw_active),edges,config['bridge_gap_sec']+step)
        groups,boundary_guards = preserve_separations(groups,raw_cores,edges,config['min_rally_sec'])
    ...
    for n, g in enumerate(groups, 1):
        ...
        supported = float(motion[s:e].mean())
        ...
        for i in range(s+3, e-3):
            if not continuous[i-3:i+4].all() or not motion[i-3:i+4].any():
                continue
        ...
        track = {'rally_id': rid, 'start_frame': s, 'last_frame': e-1,
                 ...
                 'samples': [[round(float(t[i]),6), float(xy[i,0]), float(xy[i,1]), bool(motion[i])] for i in good],
                 'sample_origins':[origins[i] for i in good],
                 'raw_ball_csv':'tracking/ball.csv','coordinate_interpolation_used':False}
        ...
        rallies.append({'rally_id': rid, 'start_sec': a, 'end_sec': b, 'duration_sec': round(b-a,3),
                        ...
                        'evidence': {'analytics_frames': [s,e-1], 'analytics_jsonl': 'analytics/detections.jsonl', 'ball_csv': 'tracking/ball.csv',
                                     ...
                                     'boundary_method': 'state_action_motion_with_low_ball' if algorithm=='fused_v2' else 'airborne_motion_with_state_audit'},
                        'uncertainty': {'low_ball_dominant':bool(low[s:e].sum()>airborne[s:e].sum()),
                            'auxiliary_associations':[x for x in associations if s<=x['frame']<e],
                            'occlusion_gaps':uncertain_gaps(t,visible,s,e,config['bridge_gap_sec']),
                            'boundary_resolution_sec':step,
                            'conservative_boundary_guards_sec':[c for c in boundary_guards if a-step<=c<=b+step],
                            'note':'回合与遮挡连续性为多信号推断，未确认得分或胜负；原始模型输出完整保留。'}})
    ...
    manifest = {'schema_version': 1, 'project': 'VolleyMole', 'source': source, 'config': config,
                'provenance': provenance, 'rallies': rallies, 'diagnostics': {
                    ...
                    'rally_algorithm':algorithm,'raw_vball_visible_frames':int(raw_visible.sum()),
                    'auxiliary_associated_frames':len(associations),'supported_low_ball_frames':int(low.sum()),
                    'carried_motion_rejected_frames':int(carried.sum()),
                    'preserved_boundary_hypotheses_sec':boundary_guards,
