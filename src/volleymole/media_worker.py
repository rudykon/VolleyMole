    ...
    with log.open('w') as errors:
        ...
        try:
            for i in range(frames):
                ...
                canvas[detail_top:header_h+detail_h]=cv2.resize(crop,(720,picture_h),interpolation=cv2.INTER_AREA)
                if lively:
                    canvas[:header_h]=composite_header(canvas[:header_h],lively[min(i,len(lively)-1)])
                else:
                    canvas[:header_h]=header_array
