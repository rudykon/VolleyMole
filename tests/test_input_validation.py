"""Real local input preflight plus deliberate error injection; no fake inference."""
import json
import subprocess
import sys
...
def test_invalid_request_is_structured_before_model_load(tmp_path, monkeypatch, kwargs):
    def forbidden(*args, **kw):
        pytest.fail("invalid request attempted to construct a detector")
    monkeypatch.setattr(engine, "VBallNetDetector", forbidden)
    result = run_detection_job(JobRequest(str(VIDEO), str(MODEL), out_root=str(tmp_path), **kwargs), analysis_id="invalid")
...
def test_ort_constructor_fallback_disabled_before_any_retry(monkeypatch, require_cuda):
    ...
    with pytest.raises(EngineError) as error:
        create_session(str(MODEL), require_cuda=require_cuda)
...
def test_config_normalization_does_not_mutate_caller_in_job(tmp_path, monkeypatch):
    polygon = [[0, 0], [100, 0], [0, 100], [0, 0]]
    req = JobRequest(str(VIDEO), str(MODEL), out_root=str(tmp_path), roi_polygon=polygon, target_fps="30000/1001")
...
def test_fingerprinting_real_file_is_cancellable():
    calls = 0
    def cancel():
        nonlocal calls
        calls += 1
    ...
    with pytest.raises(EngineError) as error:
        engine.video_fingerprint(str(VIDEO), cancel_check=cancel)
