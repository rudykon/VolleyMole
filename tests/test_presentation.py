...
from volleymole.common import Stages, save_json, digest, APP
from volleymole.presentation import build_timeline, lively_title, peak_window, validate_timeline
from volleymole.illustrated import chapter_label,rank_label,illustration_for,transition_opacity,title_lines,prepare_brand
...
class TimingTests(unittest.TestCase):
    def test_cached_stage_reports_current_hash_check_time_not_historical_cost(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch('volleymole.common.time.monotonic',side_effect=[10.,10.02]):stages.execute('render',{},callback)
...
class BrandingTests(unittest.TestCase):
    def test_logo_cache_reuses_exact_source_and_refreshes_changed_source(self):
        with tempfile.TemporaryDirectory() as temp:
            ...
            with patch('volleymole.illustrated.LOGO_SOURCE',source),patch('volleymole.illustrated.LOGO_PNG',png), \
                    patch('volleymole.illustrated.subprocess.run') as render:
