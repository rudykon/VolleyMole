...
五张材质原图位于[完整素材包](素材安装与分发.md)的 `transitions/` 下。

素材包提供材质原图，不是动画帧序列。纸艺、折光是视觉风格名称；动画采用二维合成，不是三维物理翻页或实时光线追踪。
...
把 MOV 放在两段素材上方的视频轨道，底层切点对齐 MOV 第 12 帧（从 0 开始，即 0.4 秒）。不要使用绿幕抠像，也不要先转成普通 H.264 MP4——它会丢失透明通道。先确认所用剪辑软件支持 ProRes 4444 Alpha；浏览器通常不能直接预览此编码，网页上的 MP4 用于观看效果。MOV 较大，仅导出在被 Git 忽略的 `outputs/`，约 13 MiB 原始 PNG 通过 Releases 素材包分发，仓库和 wheel 提供程序。
